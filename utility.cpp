...
string bigNumAdd2(string num1, string num2)
{
	long long sum = 0;
	string result;
...
class SegmentTreeSum { // 1-based index
...
class SegmentTreeMin { // 1-based index
	vector<ll> ST; vector<int> A; int N;
...
class SegmentTreeMax { // 1-based index
	vector<ll> ST; vector<int> A; int N;
...
public:
	...
	vector<pair<int, int> > make(char *str, int &root) {
		int i, cur = 1;
...
class BIT { // Binary Index Tree, Fenwik Tree (1-index)
public:
...
int KMP(char *s, char *p, int *pi, int s_len, int p_len) {
	int i, j, num = 0;
	i = 0; j = -1;
	pi[0] = -1;
	while (i<len_p) {
		if (j == -1 || p[i] == p[j]) { i++; j++; pi[i] = j; }
		else j = pi[j];
	}
	i = j = 0;
	while (i<len_s) {
